...
func TestDayone(t *testing.T) {
	var tests = []struct {
		...
		{DayoneInput{[]string{"1abc2", "pqr3stu8vwx", "a1b2c3d4e5f", "treb7uchet"}}, 142},
		{DayoneInput{[]string{
	...
	for _, tt := range tests {
		testname := fmt.Sprintf("Dayone(%v) - wanted: %v", tt.in, tt.res)
...
func TestDayonePartTwo(t *testing.T) {
	...
	for _, tt := range tests {
		testname := fmt.Sprintf("DayonePartTwo(%v) - wanted: %v", tt.in, tt.res)
		t.Run(testname, func(t *testing.T) {
			ans := DayonePartTwo(tt.in)
			if !reflect.DeepEqual(ans, tt.res) {
				t.Errorf("%v got: %v \n", testname, ans)
			}
		})
