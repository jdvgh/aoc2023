...
func TestDaytwo(t *testing.T) {
	var tests = []struct {
		...
		}, CubeSet{blue: 14, green: 13, red: 12},
		}, 8},
	...
	for _, tt := range tests {
		testname := fmt.Sprintf("Daytwo(%v) - wanted: %v", tt.in, tt.res)
...
func TestDaytwoPartTwo(t *testing.T) {
	...
	for _, tt := range tests {
		testname := fmt.Sprintf("DaytwoPartTwo(%v) - wanted: %v", tt.in, tt.res)
		t.Run(testname, func(t *testing.T) {
			ans := DaytwoPartTwo(tt.in)
			if !reflect.DeepEqual(ans, tt.res) {
				t.Errorf("%v got: %v \n", testname, ans)
			}
		})
